...
async function init() {
  ...
  try {
    ...
    // Nachsehen, ob es etwas Neueres gibt. Läuft im Hintergrund; ohne Netz
    // passiert nichts, und nichts davon hält den Start auf.
...
function renderStatus(st) {
  if (st.version) {
    $('appVer').textContent = 'v' + st.version;
    $('setVer').textContent = 'Version ' + st.version;
  }
...
// von Python gepusht — nach dem Start UND nach eigener Suche
window.onUpdate = (info) => {
  if (!info) return;
  const state = $('updState');
  if (info.state === 'newer') {
    const b = $('btnUpdate');
    b.textContent = `Version ${info.version} verfügbar`;
    b.classList.remove('hidden');
    b.onclick = openSettings;          // führt zum Knopf „Jetzt aktualisieren"
    state.textContent = `Version ${info.version} steht bereit.`;
    state.className = 'upd-state is-new';
    $('btnInstallUpdate').classList.remove('hidden');
    $('btnInstallUpdate').onclick = () => startInstall(info);
  } else if (info.state === 'current') {
    state.textContent = 'Sie haben die neueste Fassung.';
    state.className = 'upd-state';
  } else {
    // Beim Start bleibt das unkommentiert; nach eigener Suche nicht.
    state.textContent = manualCheck ? 'Nicht erreichbar — kein Netz?' : '';
    state.className = 'upd-state';
  }
  manualCheck = false;
};
...
function openRelease(url) {
  try { window.pywebview.api.open_release_page(url || ''); } catch (e) { /* ignore */ }
}

// Herunterladen, prüfen, einspielen — die App beendet sich danach selbst.
// Deshalb VORHER fragen: ein Programm, das unangekündigt zumacht, sieht wie
// ein Absturz aus.
function startInstall(info) {
  const ok = window.confirm(
    `Version ${info.version} herunterladen und einspielen?\n\n`
    + 'Die App wird dabei beendet und danach neu gestartet. '
    + 'Nicht gespeicherte Arbeit geht verloren.');
  if (!ok) return;
  $('btnInstallUpdate').disabled = true;
  $('updBar').classList.remove('hidden');
  $('updState').textContent = 'Wird geladen …';
  $('updState').className = 'upd-state';
  try { window.pywebview.api.install_update(); } catch (e) {
    $('updState').textContent = 'Nicht möglich: ' + errMsg(e);
    $('btnInstallUpdate').disabled = false;
  }
}

window.onUpdateStep = (s) => { if (s && s.text) $('updState').textContent = s.text; };
window.onUpdateProgress = (frac) => {
  $('updFill').style.width = Math.round((frac || 0) * 100) + '%';
};
window.onUpdateDone = (r) => {
  $('btnInstallUpdate').disabled = false;
  if (r && r.ok && r.nothing) {
    $('updState').textContent = 'Sie haben bereits die neueste Fassung.';
    $('updBar').classList.add('hidden');
    return;
  }
  if (r && r.ok) {
    $('updState').textContent = `Version ${r.version} eingespielt — die App startet neu.`;
    $('updState').className = 'upd-state is-new';
    return;
  }
  $('updBar').classList.add('hidden');
  $('updState').textContent = (r && r.msg) || 'Aktualisierung fehlgeschlagen.';
};

$('btnCheckUpdate').addEventListener('click', () => {
  manualCheck = true;
  $('btnInstallUpdate').classList.add('hidden');
  $('updState').textContent = 'Suche …';
  $('updState').className = 'upd-state';
  try { window.pywebview.api.check_update(); } catch (e) {
    $('updState').textContent = 'Nicht erreichbar.';
  }
});
...
// --- Einstellungsfenster ---------------------------------------------------
// Elf Schalter in der Kopfzeile haben die Arbeitsfläche verdrängt und liessen
// nicht erkennen, was zusammengehört. Sie stehen jetzt in einem eigenen
// Fenster; oben bleibt eine Kurzfassung, damit man ohne Öffnen sieht, was gilt.
const SET_KEY = 'ats-settings';

//: Alle Schalter mit ihrer Vorgabe — Grundlage für Merken und Zurücksetzen.
const OPTIONS = {
  optOcr: true, optLang: 'deu', optAudio: true, optCourseBg: true,
  optCleanBg: true, optGeometry: 'native', optSingle: false, optNoExams: true,
  optQuizBank: true, optQuizFont: '18.5', optQuizExport: true, optQuizSlides: false,
};
...
function saveSettings() {
  try { localStorage.setItem(SET_KEY, JSON.stringify(readSettings())); } catch (e) { /* ignore */ }
  renderSummary();
}
...
function loadSettings() {
  ...
  applySettings({ ...OPTIONS, ...saved });
}

//: Beschriftung für die Kurzfassung. Nur was eingeschaltet ist, wird genannt —
//: eine Liste aus zwölf Häkchen liest niemand.
const SUMMARY = [
  ['optOcr', 'Text editierbar'], ['optAudio', 'Audio'],
  ['optCourseBg', 'Kurs-Hintergrund'], ['optCleanBg', 'Hintergrund gesäubert'],
  ['optSingle', 'eine Szene'], ['optNoExams', 'ohne Prüfungsfolien'],
  ['optQuizBank', 'Fragenbank'], ['optQuizExport', 'Fragen als Excel'],
  ['optQuizSlides', 'Fragen als Folien'],
];
const GEO_LABEL = { native: 'Original 1:1', fit: 'Einpassen (16:9)', fill: 'Bühne füllen' };

function renderSummary() {
  const on = SUMMARY.filter(([id]) => $(id) && $(id).checked).map(([, label]) => label);
  const geo = GEO_LABEL[$('optGeometry').value] || $('optGeometry').value;
  const el = $('setSummary');
  el.textContent = '';
  const add = (text, dim) => {
    const s = document.createElement('span');
    if (dim) s.className = 'off';
    s.textContent = text;
    el.appendChild(s);
  };
  add(geo);
  on.forEach((x) => add(x));
  if (!on.length) add('keine Zusatzoptionen', true);
}
