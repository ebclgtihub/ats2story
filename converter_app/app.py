...
def _win_file_dialog(save, file_types, default_name=''):
    """Windows: nativer Datei-Dialog OUT-OF-PROCESS (PowerShell/WinForms).

    Warum nicht ``window.create_file_dialog``? pywebview 6.2.1 (winforms) ruft
    ``ShowDialog(form)`` OHNE ``InvokeRequired``/``Invoke``-Marshalling auf den
    GUI-Thread auf — anders als alle übrigen winforms-Methoden. WinForms-Dialoge
    haben Thread-Affinität; der Aufruf aus einem Worker-Thread hängt mit
    eingebettetem WebView2 die Message-Loop auf ('Not Responding'). Ein eigener
    Prozess mit eigenem STA-Thread ist davon vollständig entkoppelt und kann
    nicht deadlocken. Dynamische Werte kommen per Env-Var (keine Injektion).
    """
    if save:
