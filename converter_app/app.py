...
import updates  # noqa: E402  (liegt neben dieser Datei)
...
class Api:
    ...
    def status(self):
        return dict(ocr_lang=ats2story.ocr_lang() or 'KEINE',
                    version=ats2story.__version__)

    # -- Aktualisierung -----------------------------------------------------
    def check_update(self):
        """Nachsehen, ob es etwas Neueres gibt — im Hintergrund.

        Nicht blockierend und ohne Folgen bei Misserfolg: ohne Netz oder
        hinter einer Sperre passiert schlicht nichts.
        """
        def run():
            here = ats2story.__version__
            try:
                rel = updates.latest_release()
            except Exception:
                _log_err('check_update')
                rel = None
            if rel is None:
                # Kein Netz, Sperre, unerwartete Antwort. Beim Start schweigt
                # die Oberfläche dazu; wer selbst gesucht hat, bekommt eine
                # Antwort — sonst klickt man und es passiert nichts.
                self._push('onUpdate', dict(state='unreachable', current=here))
                return
            newer = updates.is_newer(rel['version'], here)
            self._push('onUpdate', dict(state='newer' if newer else 'current',
                                        current=here, **rel))
    ...
    def install_update(self):
        """Neue Fassung laden, prüfen und einspielen — auf Zuruf.

        Läuft im Hintergrund und meldet jeden Schritt an die Oberfläche. Die
        Datei wird gegen Größe UND Prüfsumme des Verzeichnisses gehalten;
        stimmt etwas nicht, wird sie weggeworfen und nichts eingespielt.
        """
        def run():
            try:
                if not updates.is_frozen():
                    raise updates.UpdateError(
                        'Das geht nur in der installierten App — beim Lauf aus '
                        'dem Quellcode aktualisiert git.')
                payload = updates.latest_payload()
                if not payload:
                    raise updates.UpdateError('Verzeichnis nicht erreichbar.')
                version = str(payload.get('tag_name') or '').lstrip('vV')
                if not updates.is_newer(version, ats2story.__version__):
                    self._push('onUpdateDone', dict(ok=True, nothing=True))
                    return
                asset = updates.asset_for_this_system(payload)
                if not asset:
                    raise updates.UpdateError(
                        'Für dieses System liegt keine Datei bereit.')

                self._push('onUpdateStep', dict(text=f'Lade Version {version} …'))
                tmp = tempfile.mkdtemp(prefix='ats-update-')
                path = updates.download_verified(
                    asset, tmp,
                    on_progress=lambda f: self._push('onUpdateProgress', float(f)))

                self._push('onUpdateStep', dict(text='Geprüft — wird eingespielt …'))
                if os.name == 'nt':
                    updates.apply_windows(path)
                else:
                    bundle = updates.app_bundle()
                    if not bundle:
                        raise updates.UpdateError('Programmbündel nicht gefunden.')
                    updates.apply_macos(path, bundle)
                # Erst melden, dann beenden: das Einspielen läuft ausserhalb
                # weiter und braucht uns beendet.
                self._push('onUpdateDone', dict(ok=True, version=version))
                threading.Timer(1.5, self._quit).start()
            except updates.UpdateError as e:
                self._push('onUpdateDone', dict(ok=False, msg=str(e)))
            except Exception as e:
                _log_err('install_update')
                self._push('onUpdateDone', dict(ok=False, msg=f'Unerwarteter Fehler: {e}'))
    ...
    def open_release_page(self, url: str = ''):
        """Die Release-Seite im Browser öffnen.

        Nur Adressen des eigenen Verzeichnisses — die Adresse kommt zwar aus
        unserer eigenen Abfrage, aber eine Weiche, die JEDE Adresse öffnet,
        ist eine offene Tür, sobald jemand anderes in die Oberfläche schreibt.
        """
        target = str(url or updates.PAGE)
        if not target.startswith(f'https://github.com/{updates.REPO}/'):
            target = updates.PAGE
        try:
            webbrowser.open(target)
        except Exception:
            _log_err('open_release_page')
        return None
    ...
    def _export(self, opts):
        try:
            ...
            stats = ats2story.convert_ats(
                ...
                quiz_export=bool(opts.get('quiz_export', True)),
